                ...
                if len(destination) == len(asset) and len(asset) == len(quantity):
                    ...
                    if isinstance(memo, list) and isinstance(memo_is_hex, list):
                        ...
                        if len(memo) != len(memo_is_hex):
                            ...
                            raise exceptions.ComposeError('memo/memo_is_hex lists should have the same length as sends')
                        ...
                        if not('list' in memo and 'list' in memo_is_hex and 'msg_wide' in memo and 'msg_wide' in memo_is_hex):
                            raise exceptions.ComposeError('when specifying memo/memo_is_hex as a dict, they must contain keys "list" and "msg_wide"')
                        ...
                        return mpma.compose(db, source, util.flat(zip(asset, destination, quantity, memo['list'], memo_is_hex['list'])), memo['msg_wide'], memo_is_hex['msg_wide'])
                    else:
                        # (3) the default case
