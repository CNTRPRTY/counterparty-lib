...
def _get_oldest_tx(address, block_index):
    hsh = _address_to_hash(address)
    query = {
        "method": "blockchain.scripthash.get_history",
    ...
    # result = self.send(query, timeout=timeout)
    result = _backend.send(query)
    try:
        oldest = result[0]
    except IndexError:
        return {}
    if oldest["height"] > block_index:
        return {}
    else:
        return {
            "tx_hash": oldest["tx_hash"],
            "block_index": oldest["height"]
        }
...
# def get_oldest_tx(address):
    ...
    hardcoded_key = f"{block_index}-{address}"
    if hardcoded_key in GET_OLDEST_TX_HARDCODED:
        ...
        call_result = _get_oldest_tx(address, block_index)
